defmodule TCPSpeedTest do
  defmodule Server do
    def start(port \\ 5555) do
      {:ok, listener} = :gen_tcp.open(port, [:binary, {:active, false}])
    ...
    defp serve_sock(sock, conn_id, rx_bytes) do
      ...
      case :gen_tcp.recv(sock, 0, 1000) do
        {:ok, data} ->
          serve_sock(sock, rx_bytes + byte_length(data))
        {:error, :timeout} ->
          serve_sock(sock, rx_bytes)
        {:error, err} ->
          Logger.error log_code: "sock.error", rx_bytes: rx_bytes, conn_id: conn_id
      end
    end
  end
